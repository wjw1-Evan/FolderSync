...
class FileTransfer {
    ...
    func downloadFileFull(
        ...
        try? preparePathForWritingFile(fileURL: localURL, baseDir: folder.localPath, fileManager: fileManager)
        if !fileManager.fileExists(atPath: parentDir.path) {
            ...
            if !parentRelativePath.isEmpty && parentRelativePath != "." {
                ...
                if stateStore.getState(for: parentRelativePath)?.isDeleted == true {
                    AppLogger.syncPrint("[FileTransfer] 🔄 检测到需要创建父目录，清除父目录的删除记录: \(parentRelativePath)")
                    await MainActor.run {
                        ...
                        do {
                            ...
                            guard case .chunkData(_, _, let data) = chunkRes else {
                                if case .error(let errorString) = chunkRes {
                                    AppLogger.syncPrint("[FileTransfer] ❌ 获取块数据失败: \(path) - \(errorString)")
                                }
                                return nil
                            }
                            ...
                            AppLogger.syncPrint("[FileTransfer] ❌ 下载块失败: \(path) - \(error.localizedDescription)")
                            return nil
                        }
                    }
                }
        ...
        case .error(let errorMsg) where errorMsg.hasPrefix("缺失块:"):
                    ...
                    group.addTask { [weak self] in
                        ...
                        do {
                            ...
                            if case .chunkAck = putChunkRes {
                                ...
                                AppLogger.syncPrint("[FileTransfer] ❌ 块上传失败: \(path)")
                            }
                        } catch {
                            AppLogger.syncPrint("[FileTransfer] ❌ 上传块失败: \(path) - \(error.localizedDescription)")
                        }
                        return nil
                    }
                }
            ...
            guard case .fileChunksAck = confirmRes else {
                ...
                if case .error(let errorString) = confirmRes {
                    AppLogger.syncPrint("[FileTransfer] ⚠️ 块级同步确认失败，回退到全量上传: \(path) - \(errorString)")
                }
