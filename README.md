...
- **增量同步 (CDC & Merkle Search Trees)**：
    ...
    - **状态同步 (MST)**：使用 **确定性 Merkle Search Trees (MST)** 维护集群状态，基于 **Hash-based Treap** 结构实现。确保根哈希在任何插入顺序下均保持一致，通过对比树哈希在 $O(\log n)$ 时间内快速定位差异文件。
- **一致性与冲突处理**：使用 **Vector Clocks** 追踪文件变更的因果关系。同步决策主要基于 Vector Clock 的先后关系（antecedent/successor/equal/concurrent）来决定下载或上传方向。发生并发冲突（concurrent）时，通常会自动保留多版本文件（保存为 `.conflict.{peerID}.{timestamp}` 格式）。**但为了支持文件复活（Resurrection），当本地文件的修改时间明显晚于远程删除记录的删除时间（>0.5s）时，系统会判定为“复活”操作，优先保留本地新文件并同步给对端。**对于历史上尚未携带 Vector Clock 的旧记录，则采用简单的“本地优先/冲突文件”保守策略，保证最终收敛且不丢数据。
- **设备身份与安全**：
...
- **存储机制**：使用 JSON 文件存储文件夹配置、文件索引、Vector Clocks、设备状态、同步日志，避免 SQLite 的 I/O 错误问题。**Vector Clocks 采用统一存储模式（每个 syncID 一个 vcs.json 文件），极大减少了海量小文件的 I/O 开销并提高了索引性能。**
- **块存储**：实现了块级别的去重存储系统，相同内容的块只存储一次，大幅节省存储空间。块按哈希值组织存储，支持快速查找和访问。
- **文件统计**：实现文件夹统计功能，包括文件数量、文件夹数量和总大小统计。使用流式哈希计算（64KB 缓冲区）避免大文件一次性加载到内存。支持并行处理文件（最大 8 个并发），批量处理（每 50 个文件 yield 一次）避免阻塞主线程。
...
5. **同步机制优化**：
   ...
   - **确定性 MST 比较**：MST 实现采用 Hash-based Treap，保证同步的一致性，避免因文件乱序插入导致的哈希抖动。
   - **存储性能优化**：Vector Clock 统一存储（每个 SyncID 一个 JSON），解决海量小文件同步时的磁盘 I/O 瓶颈。
   - **多对等点协同优化**：限制最大并发同步对等点数（默认为 3），并复用计算好的 MST 状态，避免在多设备环境下产生资源竞争。
...
为了确保 FolderSync 的同步逻辑在各种场景下都能保持一致性和可靠性，建议通过以下几种方式进行测试。

## 1. 自动化单元与集成测试

项目中包含了一套完整的测试套件，涵盖了从核心算法到复杂同步场景的验证。

- **核心算法测试**：
    - `MSTDeterminismTests.swift`: 验证 Merkle Search Tree 的确定性哈希。
    - `BatchVectorClockTests.swift`: 验证向量时钟的合并与比较逻辑。
    - `SyncDecisionEngineTests.swift`: 验证同步决策矩阵的正确性（upload/download/skip/conflict/uncertain）。
- **场景仿真测试**：
    - `SyncScenarioTests.swift`: 模拟多客户端（A、B、C）之间的并发修改、删除传播、递归同步等复杂场景。
    - `SyncSafetyTests.swift`: 专门针对“误删防范”、“文件复活”及其它边缘安全场景的测试。

**运行方式**：
在 Xcode 中按下 `Cmd+U` 或打开 Test Navigator 选择相应测试集运行。

## 2. 单机多实例模拟测试（高级）

FolderSync 支持通过环境变量覆盖数据存储路径，从而实现在单台 macOS 设备上启动多个完全隔离的客户端实例进行模拟对等同步。

**步骤**：
1. **编译应用**：确保 `FolderSync.app` 已成功编译在构建目录下。
2. **启动实例 A**：
   ```bash
   FOLDERSYNC_APP_DIR=/tmp/fs_node_a open -n ./FolderSync.app
   ```
3. **启动实例 B**：
   ```bash
   FOLDERSYNC_APP_DIR=/tmp/fs_node_b open -n ./FolderSync.app
   ```
4. **验证同步**：
   - 在实例 A 中添加文件夹 `/Users/yourname/TestSyncA`，设置 `syncID` 为 `test-id-123`。
   - 在实例 B 中添加文件夹 `/Users/yourname/TestSyncB`，设置相同的 `syncID` 为 `test-id-123`。
   - 在 A 中创建文件，观察 B 是否自动下载；在 B 中重命名文件，观察 A 是否同步重命名。

## 3. 跨设备手动验证

物理设备间的同步受网络环境影响，测试时请注意：
- **子网一致性**：各设备需处于同一子网，且允许 UDP 广播（用于 LAN Discovery）。
- **防火墙设置**：确保 macOS 防火墙允许 FolderSync 接收和发送网络数据。
- **PeerID 分享**：如果自动发现较慢，可手动通过“添加对等点”功能输入对端的 PeerID。

## 4. 同步验证清单 (Checklist)

在测试过程中，建议重点关注以下一致性指标：
- [ ] **哈希一致性**：同步完成后，两端文件夹内的文件内容哈希、MST 根哈希应完全一致。
- [ ] **因果有序性**：通过日志观察 Vector Clock 是否随编辑动作递增，且新版本始终覆盖旧版本。
- [ ] **冲突处理**：双方同时编辑同一文件时，应产生 `.conflict` 后缀的冲突文件，不应发生数据丢失。
- [ ] **离线追赶**：Node B 离线时 Node A 进行多次编辑，B 重新上线后能正确合并增量变更。
- [ ] **空目录与重命名**：验证空目录创建、目录嵌套重命名等操作的同步准确性。
