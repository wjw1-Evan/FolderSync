...
public class PeerRegistrationService: ObservableObject {
    ...
    private let queue = DispatchQueue(
        label: "com.foldersync.peerregistration", attributes: .concurrent)
    ...
    public func registerPeer(peerID: PeerID, addresses: [Multiaddr]) -> Bool {
        ...
        guard !addresses.isEmpty else {
            AppLogger.syncPrint(
                "[PeerRegistrationService] ⚠️ 地址列表为空，无法注册: \(peerIDString.prefix(12))...")
            Task { @MainActor in
    ...
    public func registerPeers(_ peers: [(peerID: PeerID, addresses: [Multiaddr])]) {
        ...
        for (peerID, addresses) in peers {
            ...
            guard !addresses.isEmpty else {
                AppLogger.syncPrint(
                    "[PeerRegistrationService] ⚠️ 跳过无地址的 peer: \(peerIDString.prefix(12))...")
                continue
            }
    ...
    public func retryRegistration(peerID: PeerID, addresses: [Multiaddr]) -> Bool {
        ...
        if isRegistered(peerIDString) {
            AppLogger.syncPrint(
                "[PeerRegistrationService] ✅ Peer 已注册，无需重试: \(peerIDString.prefix(12))...")
            return true
        }
