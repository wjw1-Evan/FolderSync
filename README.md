...
- **一致性与冲突处理**：使用 **Vector Clocks** 追踪文件变更的因果关系。同步决策完全基于 Vector Clock 的先后关系（antecedent/successor/equal/concurrent）来决定下载或上传方向，不再依赖文件系统或变更日志的时间戳。发生并发冲突（concurrent）时，会自动保留多版本文件（保存为 `.conflict.{peerID}.{timestamp}` 格式），用户可通过冲突中心手动解决。对于历史上尚未携带 Vector Clock 的旧记录，则采用简单的“本地优先/冲突文件”保守策略，保证最终收敛且不丢数据。
- **设备身份与安全**：
...
5. **同步机制优化**：
   - **基于 Vector Clock 的同步决策**：同步方向完全基于 **Vector Clock** 的偏序关系来决定，而不是比较文件数量、文件系统修改时间或“最后写入者胜出”这类方案。系统为每个文件维护 Vector Clock，并基于因果关系进行精确判断；对于互不可比（并发）的 Vector Clock，系统判定为冲突并保留多版本文件，由用户在冲突中心手动解决，从设计上移除了单纯依赖时间戳的同步策略。
...
6. **冲突处理**：使用 Vector Clocks 检测并发修改，自动保留冲突文件的多版本，用户可通过冲突中心手动解决。在 Vector Clock 并发（互不可比）冲突的情况下，系统不再通过时间戳自动决策方向，而是始终保留多版本冲突文件，由用户在冲突中心手动处理，彻底避免由时钟偏差带来的错误覆盖。
...
| **增量传输** | ✅ 只传输变更块 | ✅ 只传输缺失块 |
| **冲突处理** | 时间戳 + 版本号 | 纯 Vector Clocks（不依赖时间戳方案） |
...
4. **冲突处理**：使用 Vector Clocks 追踪因果关系并决定同步方向，对并发修改生成多版本冲突文件（不再依赖时间戳方案）
